...
func update_animation_state(_state: String):
	if _state == "stop":
		...
		self.play_animation()
		if _state in $Sprite.sprite_frames.get_animation_names():
			$Sprite.animation = _state
...
func _ready():
	...
	$Sprite.apply_scale(Vector2(self.spawn_dir * self.sprite_dir, 1))
...
func _physics_process(delta: float) -> void:
	...
	x_movement(movement_input, delta)
	
	# calculate vertical velocity
	jump_logic(movement_input, delta)
	apply_gravity(delta)
...
func x_movement(movement_input: Dictionary, delta: float) -> void:
	var x_dir = movement_input["x"]
...
func set_direction(x_dir: int) -> void:
	...
	$Sprite.apply_scale(Vector2(self.spawn_dir * x_dir, 1))
...
func jump_logic(movement_input: Dictionary, _delta: float) -> void:
	# Reset our jump requirements
	if is_on_floor():
		jump_coyote_timer = jump_coyote
		is_jumping = false
	if movement_input["just_jump"]:
...
#	if get_input()["jump"]:pass
	...
	if movement_input["released_jump"] and velocity.y < 0:
...
func update_health_bar() -> void:
	self.health_bar.update(self.max_health, self.health)
